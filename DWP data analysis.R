...
###~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~###
{
  V <- lambdaDWP * (x_values^2 - wellCentres^2)^2 

  scale_factor <- 10

  ggplot(diagWFData, aes(x = x)) +
    # Wavefunction (Left axis)
    geom_line(aes(y = psi0), color = "black", linewidth = 0.8) +
    
    # Potential (Right axis)
    geom_line(aes(y = V / scale_factor), color = "red", linewidth = 0.5, alpha = 0.7) +
    
    scale_y_continuous(
      name = expression("Wavefunction " ~ psi[0](x)),
      sec.axis = sec_axis(~ . * scale_factor, name = "Potential V(x)"),
      limits = c(0, 0.75)
    ) +
  
    scale_x_continuous(
    breaks = c(-wellCentres, 0, wellCentres), 
    labels = c(expression(-eta), "0", expression(eta)),
    limits = c(-3, 3)) +
...
if (save_png) { ggsave("Figures/DWP_wave_functions_potential.png", width = 8, height = 4, dpi = 1200) }
