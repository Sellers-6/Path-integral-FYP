...
const double epsilon = 0.5;				        // Maximum random displacement for Metropolis algorithm, decreasing epsilon increases acceptance rate. Want an acceptance rate between 50% and 80%. Lower rate is better for DWP so it can autocorrelate faster.
...
const int decorrSweepsDWP = 500;			        // Decorrelation takes longer in the DWP system
...
const int thermSweepsDWP = 1000;     // Thermalisation also takes longer in the DWP system
...
const int N = 1666;												// Number of lattice points. This discretises the imaginary time, so increasing N increases the accuracy of the simulation
std::vector<double> positions = std::vector<double>(N, 0.0);	// Lattice points (represents the "path" of the particle)
const double a = 0.3;											// Lattice spacing. Through the lattice spacing we define beta = N * a, the inverse temperature of the system. Making beta larger allows us to project out the ground state more effectively.
...
const double wellCentres = 1.0;     // Well centre positions, increasing this moves the wells further apart
