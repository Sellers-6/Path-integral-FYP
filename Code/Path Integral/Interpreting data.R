...
# hE0 <- hist(e0Vec, breaks = bins, main = "Histogram of ground state energies",
#      xlab = "Energy", ylab = "count")

# E0Range <- range(hE0$mids)[2] - range(hE0$mids)[1]  # Range of ground state energies 
# binWidth <- E0Range / bins                      # Width of each bin 
# E0NormFactor <- nrow(E0[[name]])                    # Normalization factor 

# E0Norm <- hE0$counts / sum(hE0$counts) / binWidth

# continuousE0 <- seq(min(e0Vec), max(e0Vec), length.out = 100) # Continuous range of energies for plotting the normal distribution

# dx <- diff(continuousE0)[1]   # spacing between points
# normDist <- dnorm(continuousE0, mean = mean(e0Vec), sd = sd(e0Vec))
# normDist <- normDist / sum(normDist * dx)  # proper normalization
...
bins <- 13  # adjust as needed

# Compute histogram breaks and bin width
hE0 <- hist(e0Vec, breaks = bins, plot = FALSE)
binWidth <- diff(hE0$breaks)[1]

# Normalized histogram counts (probability density)
E0Norm <- hE0$counts / (sum(hE0$counts) * binWidth)

# Continuous x values for normal curve
continuousE0 <- seq(min(e0Vec), max(e0Vec), length.out = 200)
normDist <- dnorm(continuousE0, mean = mean(e0Vec), sd = sd(e0Vec))
...
h <- hist(e1Vec, breaks = bins, main = "Histogram of first excited state energies",
     xlab = "Energy", ylab = "count")
